...
## 12. Prototype Validation Results — 见 [`arrival_v2_experiment_report.md`](arrival_v2_experiment_report.md)

> **2026-05-08 后**：本节原 v7 prototype validation 全部内容（§8.3 cross_u10 1M PASS / §8.2 P1 v6 1.5M PASS / 与 §8 §11 设计预言对照 / 时间预算实测 / SAC trainer resume bug 修复 / 后续工作清单）已迁出，落地为独立实验报告 [`docs/arrival_v2_experiment_report.md`](arrival_v2_experiment_report.md)。
>
> **本设计文档此后不再承载实验结果**。所有 arrival_v2 reward 实测、拓扑泛化、多 seed 复现等均写入实验报告；本设计文档仅保留 v6 spec 与历次设计修订，作为设计档案。
>
> **检索关系**：
> - 设计层（reward 公式、参数推导、不变量、§8 gate 规范、§11 校核）→ 本文件 §1–§11
> - 结果层（vanilla SAC + arrival_v2 在各 benchmark 上的实测、bug、预算偏差、follow-up）→ [`arrival_v2_experiment_report.md`](arrival_v2_experiment_report.md)
