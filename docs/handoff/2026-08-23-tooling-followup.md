...
4. ~~项目 `.claude/settings.json` 仍 gitignored，所以 [`.claude/hooks/doc_pointers.py`](../../.claude/hooks/doc_pointers.py)
   ...
   ### 装上之后立刻查出的一笔：钩子在 clone 上会挡住第一次编辑

   验收时在真 clone 里模拟了一次 hook 调用。钩子**确实触发了**——然后报 **103 处「★ 真失效」
   并 exit 2**。也就是说这条路子如果只做到上面为止，等于给每台新机器装了个开机就误伤的东西。

   根因不在文档：那 103 处指向的全是 `experiments/`、`docs/offline_mbrl_plan/`、
   `phnode_full_oc_clean/`，三者都**故意 gitignored**（`.gitignore` 第 55／118／122 行），而
   `check_doc_pointers` 判 `artifact` 桶靠的是一张写死的目录名单，只列了五个。
   **这个缺口在完整工作副本上不可能暴露**——被忽略的目录只要**在**，目标就解析得到，根本走
   不到分桶那一步。它只在缺文件的机器上发作，而在此之前没人在那种机器上跑过。

   已改成**问 git**（`git check-ignore -z --stdin --no-index`，整批一次调用），旧名单留作
   git 不可用时的兜底。验收：完整工作副本上输出与改前**逐字相同**（`diff` 全等）、clone 上
   真失效 $103\to0$、`--strict` 两侧均 exit 0。5 行负控在
   `scripts/mutations/doc_pointers_gitignore.py`，全部按预期变红。

   **改这一版时踩了三个坑，三个都是 fail-open——工具答「没有一个被忽略」，于是每一处产物引用
   都变成真缺陷：**

   | 坑 | 症状 |
   |---|---|
   | `text=True` 在 Windows 上把喂进 stdin 的 `\n` 转成 `\r\n` | git 认为每条路径尾部带 `\r`、需要引号，答回 `"experiments/foo.json\r"`；**只有最后一条**（后面没有分隔符）是干净的。读起来就是「git 说只有一条被忽略」 |
   | 批量里混进一条仓库外的路径 | git 对**整批**报 `is outside repository` 并 exit 128，我按「非 0／1 即无法作答」返回空集——**一条坏输入吞掉 154 条的答案** |
   | `foo/` 规则只匹配目录 | 对**不存在**的路径 git 无从判断它是不是目录，裸名答「不忽略」、带尾斜杠答「忽略」。这是最后一条让 clone 上 `--strict` 仍然红的 |

   顺带发现一处**名字比它能承担的更强的测试**：`test_an_artifact_path_is_not_a_defect_on_a_fresh_clone`
   早就在，却抓不到这个 bug——它的夹具用 `results/...`，**正好取自被测的那张名单**，所以它能
   证明的只有「名单里的目录被豁免」。已改名为 `test_a_path_on_the_fallback_list_is_not_a_defect`
   （名字变弱、变诚实），另补真正的 clone 用例。这是 §六第 9 条「夹具照着被测常量建输入」的
   又一实例。
