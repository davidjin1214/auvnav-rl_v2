...
## 收尾统稿轮待办（整改轮全部闭环后的唯一剩余轮次；轻流程）

> 来源：各批 findings 显式路由项。**「没提到」在末批等于「丢了」，故此表为唯一汇总入口。**

| # | 待办 | 出处 |
|---|---|---|
| 1 | **语体计量维度补做**——第 5 批 D 维度因 API 额度耗尽未执行，缺：新增文本破折号密度实测、全章正文段密度较基线 $0.593$ 的升降、top5 最长句净字数、新增超 4 句段净增计数 | findings §11 **N24** |
| 2 | 语体计量校准锚重校（全章正文段密度已越 §0.5.9 锚顶 0.60：锚要么按含-caption 口径重述、要么承认随后半章漂移） | findings §9 **N10** |
| 3 | 段落句数系统性偏离（201 段中 32 段超 4 句） | findings §9 **N11** |
| 4 | 浮动与版面：**表 5.4 距首引 3 页（本批新增，因文本增长）** ＋ 图 5.6／表 5.11／表 5.12 三处（**经 HEAD 基线独立编译比对确认为既有、非本批引入**）；另 §5.9 虚线是否移出主坐标以与 §5.7 处置完全对称 | findings §11 **N20/N19** |
| 5 | §5.8 负面结论节页数 vs §5.9 的版面配比复核 | findings §3 **M7** |
| 6 | 术语终检 ＋ 跨节引用抽核 | 惯例 |
| 7 | 残留 LOW：§5.3.5 与 §5.8.2 首半句字面重复（第 4 批复审 C）／`methodology.tex` 文献转述含「相当」（N21）／§5.7.1「消失」缺检出限定（N22）／§5.5.1「最弱者亦不低于 $0.78$」可加奖励设定限定（N23） | findings §10、§11 |
