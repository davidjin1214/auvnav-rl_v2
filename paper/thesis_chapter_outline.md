...
> 文档版本：rev.14（2026-07-28；rev.13 = 2026-07-26）
>
> **rev.14 修订要点（2026-07-28，收尾统稿轮＝本章送审前最后一轮）**：① §0.5.9 新增 **(d) 语体计量诊断读数**——写死可复现的计量口径并记录全章分布，同时**明确它是诊断读数、不是门槛**；⚠ 一并订正一处沿袭三轮的**误引**：`0.12–0.60` 这个「校准锚」**从来不在本 spec 里**，它首见于 `chapter_full_review_findings.md` 第 2 批复审 D 的自述，此后被三轮 `next_session_prompt.md` 转述为「§0.5.9 校准锚」；其数值恰是当时各节密度的 min–max，属**以被测样本自身定义合格区间**，且与 §0.5.8「撤所有量化目标」相抵，故本轮**不予采纳为阈值**。② §0.5.7 每段 3–4 句封顶**降格为诊断触发器**（与 §0.5.8「不预设段落长度的硬性目标」这一最高原则的内部冲突就此消解）；③ §0.5.11 第 2 层禁用词表补**逐实例豁免枚举**与**规则②判据**（幅度类零结果 vs 离散配置事实）；④ §0.5.11 新增**第 4 层：全称／唯一性断言的检索面**（定点复核 H-1——「§5.7.1 是全章唯一漏网」只检索了词表五词中的一词，同一形态第六次复发）＋「档案记录不得先于证据」配套（H-2）；⑤ §2 §5.10 块首次写入「**零数字字面量锁**」的实操口径（L-5：其真实约束是「每轮改动不得引入新数字」，非「本节零数字」——与 ① 的误引同属「锁只活在会被重写的文件里」）。
>
...
>
> 与现有文档的关系：
> - 本文是 thesis chapter 的**总规划**；它**统摄**而非取代 [`paper/archive/rebrac_standalone/outline.md`](archive/rebrac_standalone/outline.md)（那是 ReBRAC paper 1 的 8 页 conference outline，对应本章 §5.6–§5.8 的素材来源）与 [`paper/archive/rebrac_standalone/progress.md`](archive/rebrac_standalone/progress.md)（paper 1 LaTeX 工程进度，**31 页 arXiv preprint draft**，可作本章主干直接复用）。
...
- 每段 3–4 句封顶（沿用 paper 1 progress.md §9 R1）。⚠ **rev.14 降格为诊断触发器，不是硬帽**：该条沿用的是 paper 1 的**会议体** checklist，与 §0.5.8「本章不预设页数 / cite 条数 / **段落长度**的硬性目标……撤所有量化目标」这一**用户拍板的最高写作原则**直接冲突，冲突以 §0.5.8 为准。触发后的正确动作是**问一句**「这一段是不是在做两件不同的论证动作？」——是则拆，否则登记保留。收尾统稿轮实测：全章 198 段中超 4 句者 **39** 段（5 句 27、6 句 10、7 句 2），四轮多维度独立对抗复审**无一条 CRIT/HIGH 出自段长**；本轮据此只拆了唯一的离群点（§5.4.m 原 13 句 / 758 净字的**查阅块**，拆为 4/3/3/3、零字改动），其余登记保留。**反例一例即在本轮**：§5.8.m 的拆段初版为迁就 4 句而把「差异有三处」的第三处切到下一段，破坏枚举完整性，定点复核判为**导航性退化**，定稿改回 5+3——**这正是该条不能当硬帽的实证。**
...
**(d) 语体计量诊断读数（rev.14 新增，2026-07-28 收尾统稿轮）**

> ⚠ **先订正一处误引**：第 2 批以来的三轮 prompt 都称"§0.5.9 校准锚 `0.12–0.60`"。**本 spec 从来没有这个锚**——它首见于 `chapter_full_review_findings.md` 第 361 行（第 2 批复审 D 的自述），此后被转述为 spec 条款；上文 (c) 的校准锚是**定性**的。该数值恰是当时各节密度的 min–max，即**用被测样本自身定义合格区间**，循环；且与 §0.5.8「撤所有量化目标」相抵。故本轮**不把它写成阈值**，改记为诊断读数。

**计量口径（写死，可复现）**——三轮复审各测各的、结果无法相减，根因就是没有口径：

1. 语料 = `sections/*.tex`，按 `main.tex` 的 `\input` 序。
2. 注释：未转义 `%` 至行尾丢弃；整行注释的行整体删除（LaTeX 里整行注释**不构成分段**）。
3. 浮动：`\begin{table|figure}…\end` 整块移出正文流；其 `\caption{}` 参数经**花括号配对**抽出，单独构成 caption 流。
4. 丢弃独占一行的 `\section` `\subsection` `\label` `\input` `\clearpage` `\newpage` `\centering` `\includegraphics`。
5. **正文段** = 剩余流中由空行分隔的极大非空行块（净字 ≥5）。**行间公式不构成分段**——本章公式嵌在句中。
6. 剥宏（计数前）：依次剥 `equation|cases|align` 环境体、`$…$`、`\S\ref{}`／`\ref{}`／`\eqref{}`／`\cite*{}`／`\label{}`，再剥余下 `\macroname` 与花括号。
7. **净字** = CJK 表意字 U+4E00–U+9FFF 计数。
8. **断句**：仅 `。！？`（`；，、` 不断句）；末尾残段含 ≥1 净字则计一句。
9. **破折号**：一处 = 一个「——」（U+2014 连写两个）。密度 = 处数 ÷ 段数。

**当前读数（2026-07-28 收尾统稿轮落地后）**：全章正文段 198、破折号 122 处、密度 **0.616**；含 caption 口径 234 / 128 / **0.547**。逐节：§5.1 0.125、§5.2 0.000、§5.3 0.304、§5.4 0.182、§5.5 0.278、§5.6 0.294、§5.7 0.759、§5.8 1.037、§5.9 1.083、§5.10 1.043。三批走势（同一口径跑 git 基线）：0.663（第 3 批后）→ 0.642（第 4 批后）→ 0.629（第 5 批后）→ 0.616，**单调下降**；第 3/4/5 批新增文本破折号分别为 0/0/0。

**与历史值的关系（经独立复算订正）**：复审 D 报的 0.618（第 3 批）、0.593（第 4 批）与本口径**分子完全一致**（126、124；该计数在 160 种实现变体下恒定，对口径不敏感）。但**分母不是常数偏移**——3 位小数把分母唯一钉死为 **204** 与 **209**，对本口径的 190／193 偏移为 $+14$／$+16$，且历史批间增量 $+5$ 与全部变体的恒定 $+3$ 不相容，**无自洽实现能同时命中两点**。最贴近的单一机制是「把行间公式当作分段符」（全章 15 个 display 环境皆在段中，恒定 $+15$），但其预测 205／208 与 204／209 各差 $\mp 1$。倾向结论：**两个历史密度并非出自同一版本脚本或同一 commit 树状态。**

⚠ **因此：历史值与本口径值不可相减，甚至不可假定同源——这正是把口径写死在此处的理由。** 方向性结论不受影响：在「公式分段」变体下本章现值为 $122 \div 213 \approx 0.57$，直接取历史分母 209 亦为 $\approx 0.58$，**两种近似都低于 0.60**，即 N10「已越锚顶」在第 3 批成立、经三批累计改善**已不再成立**。

**它是诊断读数、不是门槛（援 §0.5.8）**。后四节密度高于前六节，是**论证类型差而非语体退化**：§5.7–§5.10 正是承载口径隔离、种子与配置限定、让步与 scope 句的节，而中文里这些成分的标准载体就是破折号。收尾统稿轮逐处清点后四节的 100 个破折号，**未见一处是可删的插入语堆叠**。

**真正可执行的是结构性规则**（诊断命中后照此判，而不是照数字判）：

- 破折号只承载三类成分——① 插入性限定 / 口径隔离；② 总—分同位展开；③ 因果转折收束。三类之外的，改写。
- ⚠ **凡承载口径声明、锁定措辞、scope 句、种子与配置限定者，一律不得因密度而压缩**。若某处确实臃肿，**改写而非删除**，改写后须逐句对照说明哪些成分保留了。
- 句长同理：收尾统稿轮把 §5.8.m 一个 298 净字的巨句拆为多句，**13 个成分零删减**，只改标点与段界；现全章最长句 206 净字（§5.8.1），≥120 字 39 句 / ≥100 字 99 句 / 总 702 句。
...
**规则**：每轮整改落地后、编译前，收口检索须同时执行下列各层，**逐词报出命中数与判定，零命中也须写「零命中」**。⚠ **报数须按「出现次数」而非「命中行数」**——收尾统稿轮曾按行计而把「无差异」报成 6（实为 8，定点复核 L-2）：
...
| **⚠ 上两行的语义豁免边界（rev.14，2026-07-28 收尾统稿轮实测后加）** | — | 这两行按**语义**判、不按词形判：命中而**非**方法间等价判定者合规，收尾统稿轮逐处核定的现存实例为——「相当**比例**」（量词义 ×2）、「在高位**持平**后微降」（单条曲线形态 ×2）、「**等价**于」（物理等价 ×1）与四处**否定式**「不作**等价**判断」、「**无差异**」六处全在 §5.9.1 的**预登记矩阵判读列**（N8 定稿允许）、「特权信息**消失**」（部署语义）／「协议伪象**消失**」／「失稳出现与**消失**」。本轮已把最后一处真正的规则②漏网（§5.7.1「回落却随行为约束形式而消失」）改为「**可检测的**回落却随行为约束形式而**不再出现**」，并把 §5.2 文献转述的「相当的效果」改为「有竞争力的效果」（属他人结论转述、原判合规，改的是词形，以使该词表可报真零命中）。**下一轮复检时，上列合规实例不必重新裁定。**<br>⚠ **豁免枚举补全（定点复核 L-1）**：上列初版漏了三类，一并登记——「**不低于**」3 处（原始读数陈述 ×2 ＋ 第 3 批 H2 **锁定的正向锚**「点估计不低于特权协议参照」×1）；「**不再带来**」1 处（`td3bc.tex` §5.6.2，所指为**最优权重退回 $\alpha_{\mathrm{BC}}\to 0$** 这一离散配置事实）；「**未再出现**」2 处（`rebrac.tex` §5.7.2 同属配置事实；`discussion.tex` §5.10.3 已带「可检测的」）。<br>⚠ **规则②的判据（本轮补做逐实例裁定后写死）**：规则②管的是**幅度类零结果**；**离散配置事实**（后验选择的最优配置落在哪一档）不在其射程内，但须把所指写成**可直接观测的形式**，不得用带解释色彩的短语代指——`rebrac.tex` §5.7.2 即因此把「『价值项无益于可部署策略』的退化」改为「最优配置退回纯行为克隆这一退化」。 |
...
> **第 4 层 · 全称与唯一性断言的检索面（rev.14 新增，2026-07-28 收尾统稿轮定点复核 H-1 后加）**：凡写下「**唯一／全部／均未／各节都**」这类**全称或唯一性**断言，**须先证明检索面覆盖该断言的全部射程**，并在呈报中写明检索了哪些词、哪些节。
>
> 病史：第 5 批在协议之家写下「本章各结果节均未作等价判断」，被 §5.9.1 的表格判读列当场反证（N8）；收尾统稿轮写下「§5.7.1 是全章唯一的规则②漏网」，而检索**只穷举了词表五词中的一词**，遗漏 `rebrac.tex` §5.7.2 一处同结构裸用——且该错误结论已同时写进四份文档。**这是同一形态的第六次复发**，退化点已由「词表不全」（N18）、「粒度不对」（N25）移到「**断言的量词强于检索的覆盖**」。
>
> **配套：档案记录不得先于证据。** 凡引述他方复核（子 agent、独立复审）结论的句子，**一律等结论到手后再写**；写入前须能指出该结论的出处。收尾统稿轮曾把「反 churn 子 agent 独立复核同结论」写在该复核返回之前，且恰好写错（findings §12.9 教训 1）。
...
- **⚠「§5.10 零数字字面量锁」的实操口径（rev.14 首次写入 spec，2026-07-28 收尾统稿轮定点复核 L-5）**：该锁此前**只存在于 `discussion.tex` 头注与每轮 prompt**，spec 内无记载——与 §0.5.9 (d) 订正的「0.12–0.60 校准锚」是同一类问题（**锁只活在会被重写的文件里**），故一并落 spec。**其真实约束是「每轮改动不得引入新的数字字面量」，不是「本节零数字」**：`discussion.tex` §5.10.3 正文与表 5.21 现含「点估计约 $6$ 个百分点」等既有读数，先于该锁存在且历经五批复审，均为**收束层复述源头节已刊值**、非新增 headline 数字。按字面读该锁会在下一轮产生假阳性，故在此写死口径。新增数字仍须回查 ground truth 并经用户批准（第 2 批曾为 §5.8.1 破例一次）。
