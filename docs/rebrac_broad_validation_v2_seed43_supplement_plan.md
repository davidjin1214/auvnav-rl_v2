...
> **✅ 裁决与落地追注（2026-08-17 补记于此，与 B.2 追注对称）**：本呈报的裁决在**头注第三行**（2026-07-12：道 1 选 (a)）与**附录 C 头注**（2026-07-19 已批并落地）中已记，但本节此前没有就地追注，读到这里的人会以为仍悬着。实际结果：用户批 (a)，且按其修正**不作分批执行的方法学论证**——2026-06-18 转录值直接定性为**引用错误、完全作废**（视作占位数字，不比对两次结果差异）。上文 (c) 的三项保留状态**均已解除**：§7.10 由 PARTIAL 3/9 重写为 ✅ 9/9 本机实核，E·M1 已勾销，`.tex` 侧 `online.tex` rev.7 / `boundary.tex` rev.4 已按新值落地。落地清单与定点复审 PASS 记录见附录 C 头注。**本节 §B.1 的判读分析（含 1–4 点转录疑点推理）仅作存档**，其中「混用规则须批」相关论证已按附录 C 追注作废。
