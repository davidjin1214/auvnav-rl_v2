...
| 临界传感 3-seed 补录 `docs/arrival_v2_experiment_report.md` | ⏳ 用户已裁决 (a)（2026-07-12：补跑值为新 ground truth，走「新的事实性问题」专轮）；**重排方案呈批稿已起草（plan 附录 C），待批**——获批前 §7.10、`online.tex`/`boundary.tex` §5.8.4、fig 脚本一字不动，E·M1 不勾销。核心重排：s0 0.26±0.15→0.46±0.39、s1 0.90±0.00、s2 0.87±0.03、gap 0.61→0.44；已核 manifest floor 27/30 与 §5.10 零波及 | findings E·M1；plan 附录 B.1（呈报）/ C（方案） |
