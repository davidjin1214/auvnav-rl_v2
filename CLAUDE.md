...
| [`docs/tracebacks/README.md`](docs/tracebacks/README.md) | **Before re-deriving where a published number came from, look here.** Four hand traceback passes (2026-05…08) each cost a session and left nothing re-runnable; these specs make them one command (`python -m scripts.audit_published_numbers`). `--ddof` is also the only mechanical answer to "which dispersion convention is this `±`" for the `docs/*.md` side — `ch5_dispersion_audit.py` covers only `.tex` rows that print their own per-seed values. All five chains are covered; the README's own table says with how many claims each, and what each deliberately leaves out. The fifth, `online_a0`, is **not** one of the four hand passes — it is the online line's A0 sensor screen, traced de novo in 2026-08-24, and its provenance rule was established by recomputation rather than read off the summary's prose |
