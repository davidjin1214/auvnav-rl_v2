...
| 词 | 为何是错 | ✓ 正文用语 |
|---|---|---|
| **teacher / online teacher** | 引进了不存在的**知识蒸馏师生关系**：离线策略学自 baseline 采集的数据，不蒸馏在线策略；该在线策略在 §5.4 是**性能上界/参照**、§5.8 是**数据采集器** | "**以特权信息在线训练所得策略（用作性能上界/参照基准）**"；不造"特权教师"。"distill/蒸馏"仅在真有蒸馏时用 |
...
**校准锚（中心命题定稿 — 候选一，可直接入 §5.1 / §5.9；废止旧"可用性与匹配"表述）**
...
以本句的**动词对比**（用好已有 vs 增添能力）作为全章中心命题的统一表述；§5.1 中心句、§5.9 本章小结、各节首句的"本节贡献哪一块"自查均回归这条线索。
