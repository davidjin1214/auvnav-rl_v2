...
% rev.4（2026-07-26）：全章复审整改第 5 批（重流程；findings §10 N13 连锁，复审 B/C 判 HIGH）。
%   §5.4.m 一句。setup.tex rev.6 把「终检评估」的定义锚回**评估时机**，并声明「检查点身份」
%   是另一条轴、由各结果节标明、规则之家为本节；但本节原写「各方法训练内的检查点**统一**按
%   验证集…字典序选择」这一**全称规则**，被 §5.5.m／§5.8.m／§5.9.m 三节明文否定
%   （「不作验证集选点，直接取训练终点检查点」）——读者按 §5.3.6 的指针追到本节，只会拿到
%   与三节矛盾的规则。→ 改为「按两种规则之一确定，由各结果节按其考察目的选取并在实施细节中
%   标明：以最优配置为目标的单元设验证集选点…；以失效边界或对称对照为目标的单元不设选点，
%   直接取训练终点检查点」，跨种子超参后验选择仍一律按同一字典序。零数字/零论证改动。
...
%   规则），公式与论证零改动。
% 收尾统稿轮（2026-07-28）：§5.4.m 一处，**只插三个空行，零字改动**。
%   病征（本轮计量实测）：该段 13 句 / 758 净字，是全章唯一的段句数离群点（次长段
%   为 7 句；净字为次长段的 2.4 倍）。判断：它不是论证段而是**查阅块**——在 NO
%   APPENDIX、正文不用 itemize 的章里，段界是读者仅有的导航手段。
%   切点（四段，4/3/3/3 句）：① 符号记法（状态观测动作…、终止标志与 §5.3 记号互斥、
%   后继动作、行为约束系数）；② 求和/取均值的常数因子归并 + Q 归一化下界 ε；
%   ③ 确定性分支共同实现口径（目标平滑/噪声截断/延迟更新、单 Q 与双 Q 的分工、
%   Q_φ 约定）+ 特权观测与离线记录指针；④ 检查点两规则（第 5 批 N13 定稿）+ 跨种子
%   后验选择字典序 + 超参数指针。
%   ⚠ 反 churn：第 5 批 N13 写入的「各方法训练内的检查点按两种规则之一确定，各结果节
%   在实施细节中标明所用者」完整落在第 ④ 段内，一字未动。
% =====================================================================
...
行为约束系数中，$\alpha_{\mathrm{BC}}$ 为 TD3+BC 的单一行为克隆权重，$\beta_1,\beta_2$ 分别为 ReBRAC-Q 策略一侧与价值一侧的行为约束权重，$\alpha_{\mathrm{FQL}}$ 为 FQL 的蒸馏模仿权重，$\alpha_{\mathrm{ent}}$ 为 SAC 的熵温度系数。各方法的二次模仿与支持惩罚项（含策略一侧的原始动作模仿与价值一侧的下一步支持惩罚）在实现上或按动作维求和、或按动作维取均值，二者相差一个与动作维数有关的常数因子，已并入相应的行为约束系数，不影响其作为强度调节量的含义。Q 归一化中的下界 $\varepsilon$ 仅为数值稳定而设，不承担任何调节作用。

确定性分支共用的目标策略平滑噪声、噪声截断幅度与延迟策略更新频率，作为 TD3 一系方法的共同实现口径，在各离线方法中保持一致；其中 TD3+BC 的策略改进项按其原始实现取单一价值网络的估计；其余方法（SAC、ReBRAC-Q 与 FQL）的策略改进项，以及所有方法的自举目标，均取双价值网络的较小值。各式中出现于策略改进项的 $Q_\phi$ 均按此约定理解。特权观测的维度与物理含义已在 \S\ref{subsec:ch5_obs_privileged} 给定，本节不再重复其推导；离线数据集为支持特权协议，随每条转移一并记录特权观测及其后继（\S\ref{subsec:ch5_impl}）。

各方法训练内的检查点按两种规则之一确定，各结果节在实施细节中标明所用者：以最优配置为目标的单元设验证集选点，按成功率、回报、负安全代价与负用时的字典序选择；以失效边界或对称对照为目标的单元不设选点，直接取训练终点检查点。跨随机种子的超参数后验选择一律按相应验证指标的同一字典序进行，以避免人工挑选或事后口径漂移，各结果节只补充本节特有的后验选择对象。各方法的具体超参数取值、数据集规模与统计结果，随相应结果节给出。
