...
| [`docs/data_integrity_open_items.md`](docs/data_integrity_open_items.md) | ⚠ Bookkeeping issues against those numbers. Items ①③⑤ were verified and disclosed in the chapter on 2026-08-16 (route ①-c — disclose in the prose, report the sensitivity readouts). The contamination enumeration closed repo-wide on 2026-08-21 (Drive-side, 35 datasets × 24 manifests, exit 0). **The catch that outlives the closure:** 13 of the 24 evaluation manifests it rested on exist only on Drive and were never committed, so `python -m scripts.audit_seed_overlap` from a clone silently covers a smaller surface than the run that closed it — point `--data-dir` / `--benchmarks-dir` at the Drive mount to reproduce |
