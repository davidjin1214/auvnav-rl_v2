...
> ⚠ **数据完整性限定（2026-08-20 ① 层深核补注，零数字改动）**
>
> 本报告是第 5 章引数的权威源，但它写于 2026-05-01，比两条已确证的记账问题早。引本文任何读数前先读 [`data_integrity_open_items.md`](data_integrity_open_items.md)：
>
> - **①**：`crosscomp-2000` 的采集种子区间 0..1999 完整包含评估 manifest 的 1250..1349，**100/100 任务实例逐条相同**（2026-08-09 确证）。因此 §1 / §7.7 / §10 里那条**翻转**——`ep2000 (0.918) > ep1000 (0.902)`——是全文受污染最直接的断言。干净 manifest（种子 3000..3099，与训练与评估区间均不相交）上原封不动重跑同批检查点后：翻转由 $+1.6$ pp 收为 $+0.8$ pp（$t=0.43$）、可归因于 ① 的抬高量为差中差 **$+0.80$ pp**（95% CI 跨零），**方向仍为正、结论未被推翻**；但两格绝对水平均下移 $4.0$/$4.8$ pp（五种子无一反向）。
> - **③**：选 checkpoint 用的 40 回合验证集是终检 100 回合集的**固定前 40 条**；40+40 的单元两份 manifest 完全相同，即**筛查单元没有留出集**。仅计入留出 60 条的复核读数见 `rebrac.tex` §5.7.m（本文 §7.10 / §7.16 的差距闭合 53.0%/48.5% 在留出集上为 48.9%/35.6%）。
>
> 本报告的数字**一律不改**：它们在自己的评估口径下均为实测值，且 §7.8 两条 Rule 是预登记判据（改值等于篡改判据）。变的只是**引用时必须同时带上的限定**。章内已就地披露于 `setup.tex` §5.3.5/§5.3.6、`rebrac.tex` §5.7.1/§5.7.2/§5.7.m 与 `discussion.tex` §5.10.4；复算工具 [`ch5_clean_probe_readout.py`](../paper/thesis_ch5/tools/ch5_clean_probe_readout.py)、[`ch5_holdout_split_audit.py`](../paper/thesis_ch5/tools/ch5_holdout_split_audit.py)。
