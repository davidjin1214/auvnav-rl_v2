...
1. **`crosscomp-2000` 的训练与评估用的是同一批任务实例**（数据集 `seed=0`、$2000$ 回合 → 种子 0..1999 完整包含评估 manifest 的 1250..1349；reset RNG 重放确认 **100/100 实例逐条相同**）。波及 §5.7 结果表 $2000$ 回合一格与 §5.7.1「翻转」论述（$0.918$ 对 $0.902$，仅 $+1.6$pp）。§5.6「$2000<1000$」方向与污染相反，**不受威胁**。~~全仓仅此一个数据集受影响~~ —— **2026-08-16 Drive 侧探针推翻**：含噪 $2000$ 集同样 `seed=0`、100/100 逐条相同（新增第 ⑤ 条），且该次审计的目录枚举本身残缺，污染面**仍未封闭**。
2. **选点验证集是终报测试集的前缀子集**（manifest 生成器无 seed 偏移 + val/test 同 benchmark key → val_40 = test_100 的前 40 条；$40+40$ 的单元两份完全相同）。正文未写明这一包含关系（**注意：正文从未写过「独立测试集」，见下方订正**）。
...
**Drive 侧首轮探针已回（2026-08-16，`notebooks/ch5_data_integrity_probe_completed.ipynb`）**：

- **探针 A 通过** —— cross-1000／cross-2000 两格 × 5 种子共 10 个**选中**检查点全部在位 → 处置路径 **①-c（在现有检查点上补一次干净终检）成立**，不必重采重训。
- **探针 B 钉死第 ⑤ 条** —— 含噪 $2000$ 集 `seed=0`、$2000$ 回合，与评估 manifest **100/100 任务实例逐条相同**。于是 §5.6.2 那句「纯 BC 由约 $0.60$ 升至约 $0.74$」**两个端点都坐在污染数据上**，且该筛查是 $40+40$ 单元 → ①③⑤ 在这一句上同时命中。
- **探针 B 自曝枚举残缺** —— range pass 只列出 24 个数据集、漏掉了那个确实存在且同样污染的含噪 $2000$（`--verify` 按路径直读得到了它）。→ 「只有一个数据集受影响」**不成立**，污染面未封闭，须以 listdir 重建全集后重判（修复格已加进 notebook §2）。
- **新发现：① 与 ③ 在 2000 格上叠加** —— 该格选点用的 40 条 val（`offline_rebrac_broad/val_40/`）本身就在训练区间内。即"在训练 episode 上选点、再在训练 episode 上报告"。波及面评估与独立复核均未单列此点。

**①-c 补评已完成，污染幅度 $\delta$ 实测到手（2026-08-16）**：干净 manifest（种子 3000..3099，与训练 0..1999 及现测试集 1250..1349 均不相交）上重跑原封不动的 10 个选中检查点。复算工具 [`tools/ch5_clean_probe_readout.py`](tools/ch5_clean_probe_readout.py)。

| 量 | 已刊 | 干净集 | 位移 |
|---|---|---|---|
| ReBRAC-Q cross-1000 | $0.902\pm0.021$ | $0.862\pm0.019$ | $-4.0$ pp（t=4.78，5/5） |
| ReBRAC-Q cross-2000 | $0.918\pm0.030$ | $0.870\pm0.025$ | $-4.8$ pp（t=4.71，5/5） |
| **翻转（2000−1000）** | $+1.6$ pp（t=1.06） | $+0.8$ pp（t=0.43，3/5 不为负） | — |
| 采集器基线（无训练无选点） | $0.950$ | $0.930$ | $-2.0$ pp（SE≈3.4） |

- **① 可归因的抬高量 = 差中差 $+0.80$ pp**（t=0.512，CI $[-3.5,+5.1]$）→ 低于复核预设的 $2$ pp 线、远低于 $5$–$7$ pp 线。**「未再检出回落」在干净集上依然成立。**
- **但两格绝对水平都掉 $4$–$5$ pp 且高度显著**，其中仅约 $2$ pp 是抽样难度；余下约 $2$–$3$ pp 是**选点族泛化**——检查点是在种子 1250 族的 `val_40` 上选的。**`hold60` 结构上看不见这一层**（留出 60 条仍属同族），故"③ 已零成本量化完毕"须收窄为"③ 的已刊内效应已量化"。
- **只补评了 ReBRAC-Q 两格**；TD3+BC / 纯 BC 未重评 → §5.7.1 的基线差距（$23.0$ / $32.2$ pp）**不能**在干净集上重述。

**裁决前不动任何 `.tex`；裁决后 ①③⑤ 合成一批整改，重流程复审只付一次。**
