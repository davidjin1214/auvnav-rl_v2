...
我们把已做实验的边界与延伸方向整理为以下 12 条。每条都对应 \S\ref{sec:method}--\S\ref{subsec:ablations} 中某个 claim 的边界条件，目的是\emph{不让审稿人替我们 imagined} 一个我们没做但暗示能做的实验。
...
\begin{itemize}
    ...
    \item \textbf{L7 (Sensor protocol coverage).}\quad 全部实验固定 \texttt{probe-layout = s0}（单点 DVL 水追踪）。我们没有在 \texttt{s1} (DVL + 短程 ADCP) 或 \texttt{s2} (DVL + 长程 ADCP + 侧向波束) 协议下重复 ReBRAC-Q vs.~TD3+BC 对比 —— 在 sensor 信息更丰富时 deployable→teacher gap 是否仍存在、dual penalty 是否仍必要，本文未回答。
    ...
    \item \textbf{L12 (Regime/reward scope; critical-regime boundary).}\quad 本文 finding 1--4 限定在 \emph{sub-critical} 流场（$\mathrm{Re} = 150$，cross-stream）与 \emph{efficiency-v2} reward 下；表~\ref{tab:main_table} 全部三个 dataset 均处此 scope。在更难的 \emph{critical} 流场（$\mathrm{Re} = 250$，$U_\infty = 1.5$ m/s）+ \emph{arrival-v2} reward 下，我们用 oracle privileged-teacher 采集的 1000-episode 离线数据训练同口径 ReBRAC-Q ($\beta_1 = 4$, $\beta_2 = 2$)，两个 seed 上 test success 均为 $0.000$（rule-of-three 95\% 上界 $\approx 0.05$ over $60$ pooled episodes），且比 vanilla SAC online ($\sim 10\%$ catastrophic floor) 反而低 $\sim 10$pp。一个 \emph{asymmetric-critic ablation}（唯一变量：critic 输入扩展为 $\mathbf{o}_t^{\mathrm{priv}}$ \eqref{eq:setup_priv_obs} 的 \texttt{AsymmetricQNetwork}；actor 仍只读 \texttt{s0}，actor-improvement 阶段 priv 通道 zero-pad 以保部署对齐；逐 episode 配对）同样 $0.000/0.000$，paired episode-level mean $\Delta$progress 仅 $1.15$ SEM (n.s.)、median $\Delta = -0.016$，与 baseline 部署行为在统计上不可区分。这\emph{排除}了 critic 价值估计失败作为天花板成因（critic-fundamental hypothesis），与一个 actor-side partial-observability ceiling 一致并\emph{加强}本文 finding 2 (\S\ref{subsubsec:finding_2}) 的 actor-centric 解读 —— 但\emph{不}证明 \texttt{s0} actor 信息论上不可学（asym-critic 机制在 offline + actor-update zero-pad 下也可能未把 privileged 信号有效转化）。Future work 指向 actor 侧更强表征：history-stacking、recurrent policy、学到的 flow-estimator。完整 cell-level 配对数字、verdict gate 与 honest causal scope 见 supplementary materials\footnote{Standalone broad-validation v2 报告与 N2$'$ asym-critic ablation 配对分析：\texttt{docs/rebrac\_broad\_validation\_v2\_report.md}，\S 3--\S 5（broad-val v2 N0 + N2$'$）与 \S 4.5（asym-critic ablation）。}。
\end{itemize}
